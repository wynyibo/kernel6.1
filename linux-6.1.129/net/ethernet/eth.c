...
__be16 eth_type_trans(struct sk_buff *skb, struct net_device *dev)
{
	...
	//跳过 14 字节的以太网头部，让 skb->data 指向 网络层（IP/ARP）数据
	 ...
	 */
	//DSA机制处理（Linux处理交换芯片机制）
	if (unlikely(netdev_uses_dsa(dev)))
		return htons(ETH_P_XDSA);
	//判断是否是标准以太网（802.3 以太网帧）
	 ...
	 */
	//处理 Novell NetWare IPX 非标准 802.3 帧
...
{
	memcpy(((u8 *)hh->hh_data) + HH_DATA_OFF(sizeof(struct ethhdr)), haddr,
	       ETH_ALEN);
}
...
static int fwnode_get_mac_addr(struct fwnode_handle *fwnode, const char *name,
			       char *addr)
{
	int ret;
