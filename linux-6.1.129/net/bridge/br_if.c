...
void br_port_carrier_check(struct net_bridge_port *p, bool *notified)
{
	...
	if (!(p->flags & BR_ADMIN_COST) && netif_running(dev) &&
	    netif_oper_up(dev))
...
int br_add_bridge(struct net *net, const char *name)
{
	struct net_device *dev;
	int res;
	//分配一个net_device结构体，并初始化为 bridge 设备，br_dev_setup启动网桥
	dev = alloc_netdev(sizeof(struct net_bridge), name, NET_NAME_UNKNOWN,
			   br_dev_setup);

	if (!dev)
		return -ENOMEM;
	// 关联 net_device 到指定的 network namespace
	dev_net_set(dev, net);
	// 绑定网桥设备的 RTNL link 操作（用于 netlink 通信）
					...
					netdev_features_t features)
{
	...
	list_for_each_entry(p, &br->port_list, list) {
		features = netdev_increment_features(features, p->dev->features,
						     mask);
	}
