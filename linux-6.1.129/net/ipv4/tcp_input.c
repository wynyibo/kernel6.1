...
#define FLAG_RETRANS_DATA_ACKED \
	0x08 /* "" "" some of which was retransmitted.	*/
...
static void bpf_skops_parse_hdr(struct sock *sk, struct sk_buff *skb)
{
	bool unknown_opt =
		...
		BPF_SOCK_OPS_TEST_FLAG(
			tcp_sk(sk), BPF_SOCK_OPS_PARSE_UNKNOWN_HDR_OPT_CB_FLAG);
	bool parse_all_opt = BPF_SOCK_OPS_TEST_FLAG(
		tcp_sk(sk), BPF_SOCK_OPS_PARSE_ALL_HDR_OPT_CB_FLAG);
...
static void tcp_measure_rcv_mss(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (len >= icsk->icsk_ack.rcv_mss) {
		icsk->icsk_ack.rcv_mss =
			min_t(unsigned int, len, tcp_sk(sk)->advmss);
		/* Account for possibly-removed options */
		if (unlikely(len >
			     icsk->icsk_ack.rcv_mss + MAX_TCP_OPTION_SPACE))
...
static void tcp_incr_quickack(struct sock *sk, unsigned int max_quickacks)
{
	...
	unsigned int quickacks =
		tcp_sk(sk)->rcv_wnd / (2 * icsk->icsk_ack.rcv_mss);
			     ...
			     unsigned int skbtruesize)
{
	...
	int window =
		tcp_win_from_space(
			sk, READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_rmem[2])) >>
		1;
...
static void tcp_init_buffer_space(struct sock *sk)
{
	...
	if (tp->window_clamp >= maxwin) {
		...
		if (tcp_app_win && maxwin > 4 * tp->advmss)
			tp->window_clamp = max(maxwin - (maxwin >> tcp_app_win),
					       4 * tp->advmss);
	}

	/* Force reservation of one segment. */
	if (tcp_app_win && tp->window_clamp > 2 * tp->advmss &&
...
static void tcp_clamp_window(struct sock *sk)
{
	...
	if (sk->sk_rcvbuf < rmem2 && !(sk->sk_userlocks & SOCK_RCVBUF_LOCK) &&
...
static void tcp_update_pacing_rate(struct sock *sk)
{
	...
	if (tcp_snd_cwnd(tp) < tp->snd_ssthresh / 2)
		...
		rate *= READ_ONCE(
			sock_net(sk)->ipv4.sysctl_tcp_pacing_ca_ratio);
 ...
 */
static u32 tcp_dsack_seen(struct tcp_sock *tp, u32 start_seq, u32 end_seq,
				      ...
				      const int ts)
{
	...
	if ((metric > tp->reordering * mss) && mss) {
		...
		pr_debug("Disorder%d %d %u f%u s%u rr%d\n", tp->rx_opt.sack_ok,
			 inet_csk(sk)->icsk_ca_state, tp->reordering, 0,
			 tp->sacked_out,
			 tp->undo_marker ? tp->undo_retrans : 0);
#endif
		tp->reordering = min_t(
			u32, (metric + mss - 1) / mss,
			READ_ONCE(
 ...
 */
static void tcp_notify_skb_loss_event(struct tcp_sock *tp,
				      const struct sk_buff *skb)
{
...
static u8 tcp_sacktag_one(struct sock *sk, struct tcp_sacktag_state *state,
			  u8 sacked, u32 start_seq, u32 end_seq, int dup_sack,
			  int pcount, u64 xmit_time)
{
	struct tcp_sock *tp = tcp_sk(sk);
	...
	if (dup_sack && (sacked & TCPCB_RETRANS)) {
		if (tp->undo_marker && tp->undo_retrans > 0 &&
			...
			tp->undo_retrans =
				max_t(int, 0, tp->undo_retrans - pcount);
		if ((sacked & TCPCB_SACKED_ACKED) &&
			    ...
			    bool dup_sack)
{
	 ...
	 */
	tcp_sacktag_one(sk, state, TCP_SKB_CB(skb)->sacked, start_seq, end_seq,
			dup_sack, pcount, tcp_skb_timestamp_us(skb));
...
static int tcp_sack_cache_ok(const struct tcp_sock *tp,
			     const struct tcp_sack_block *cache)
{
				   ...
				   struct tcp_sacktag_state *state)
{
	...
	const unsigned char *ptr =
		(skb_transport_header(ack_skb) + TCP_SKB_CB(ack_skb)->sacked);
	struct tcp_sack_block_wire *sp_wire =
		(struct tcp_sack_block_wire *)(ptr + 2);
	...
	found_dup_sack = tcp_check_dsack(sk, ack_skb, sp_wire, num_sacks,
					 prior_snd_una, state);
...
static inline bool tcp_packet_delayed(const struct tcp_sock *tp)
{
	...
	if (tp->retrans_stamp && tcp_tsopt_ecr_before(tp, tp->retrans_stamp))
...
static void DBGUNDO(struct sock *sk, const char *msg)
{
	...
	if (sk->sk_family == AF_INET) {
		pr_debug("Undo %s %pI4/%u c%u l%u ss%u/%u p%u\n", msg,
			 ...
			 tcp_snd_cwnd(tp), tcp_left_out(tp), tp->snd_ssthresh,
			 tp->prior_ssthresh, tp->packets_out);
	}
...
static bool tcp_try_undo_loss(struct sock *sk, bool frto_undo)
{
	...
	if (frto_undo || tcp_may_undo(tp)) {
		...
		if (frto_undo)
			NET_INC_STATS(sock_net(sk), LINUX_MIB_TCPSPURIOUSRTOS);
			     ...
			     int *rexmit)
{
	...
	if ((flag & FLAG_SND_UNA_ADVANCED ||
	     rcu_access_pointer(tp->fastopen_rsk)) &&
...
static bool tcp_ack_update_rtt(struct sock *sk, const int flag, long seq_rtt_us,
...
static void tcp_send_challenge_ack(struct sock *sk)
{
	...
	if (now != READ_ONCE(net->ipv4.tcp_challenge_timestamp)) {
		...
		WRITE_ONCE(net->ipv4.tcp_challenge_count,
			   half + prandom_u32_max(ack_limit));
	}
...
static void tcp_process_tlp_ack(struct sock *sk, u32 ack, int flag)
{
	...
	} else if (!(flag & (FLAG_SND_UNA_ADVANCED | FLAG_NOT_DUP |
			     FLAG_DATA_SACKED))) {
				      ...
				      bool exp_opt)
{
	...
	if (len >= TCP_FASTOPEN_COOKIE_MIN && len <= TCP_FASTOPEN_COOKIE_MAX)
		...
		default:
			...
			case TCPOPT_TIMESTAMP:
				if ((opsize == TCPOLEN_TIMESTAMP) &&
					...
					opt_rx->rcv_tsecr =
						get_unaligned_be32(ptr + 4);
				}
				break;
			case TCPOPT_SACK_PERM:
				if (opsize == TCPOLEN_SACK_PERM && th->syn &&
				    !estab &&
				    READ_ONCE(net->ipv4.sysctl_tcp_sack)) {
			...
			case TCPOPT_FASTOPEN:
				tcp_parse_fastopen_option(
					opsize - TCPOLEN_FASTOPEN_BASE, ptr,
					th->syn, foc, false);
				break;
...
static bool tcp_parse_aligned_timestamp(struct tcp_sock *tp,
					const struct tcphdr *th)
{
...
static int tcp_disordered_ack(const struct sock *sk, const struct sk_buff *skb)
{
	...
	return (/* 1. Pure ACK with correct sequence number. */
		(th->ack && seq == TCP_SKB_CB(skb)->end_seq &&
		 seq == tp->rcv_nxt) &&
 ...
 */
static bool tcp_try_coalesce(struct sock *sk, struct sk_buff *to,
...
static bool tcp_ooo_try_coalesce(struct sock *sk, struct sk_buff *to,
				 struct sk_buff *from, bool *fragstolen)
{
 ...
 * simplifies code)
 */
static void tcp_collapse(struct sock *sk, struct sk_buff_head *list,
...
restart:
	...
	while (before(start, end)) {
		...
		if (list)
			...
			__skb_queue_tail(&tmp,
					 nskb); /* defer rbtree insertion */
	...
	skb_queue_walk_safe(&tmp, skb, n) tcp_rbtree_insert(root, skb);
}
...
new_range:
	...
	for (head = skb;;) {
		 ...
		 */
		if (!skb || after(TCP_SKB_CB(skb)->seq, end) ||
...
static void tcp_urg(struct sock *sk, struct sk_buff *skb,
		    const struct tcphdr *th)
{
	struct tcp_sock *tp = tcp_sk(sk);
	...
	if (unlikely(tp->urg_data == TCP_URG_NOTYET)) {
		u32 ptr =
			tp->urg_seq - ntohl(th->seq) + (th->doff * 4) - th->syn;
				  ...
				  const struct tcphdr *th, int syn_inerr)
{
	...
	if (tcp_fast_parse_options(sock_net(sk), skb, th, tp) &&
	    tp->rx_opt.saw_tstamp && tcp_paws_discard(sk, skb)) {
		if (!th->rst) {
			NET_INC_STATS(sock_net(sk),
				    ...
				    struct tcp_fastopen_cookie *cookie)
{
	...
	if (data) { /* Retransmit unacked data in SYN */
		...
		skb_rbtree_walk_from(data) tcp_mark_skb_lost(sk, data);
		tcp_non_congestion_loss_retransmit(sk);
...
{
	struct tcp_sock *tp = tcp_sk(sk); // 获取 tcp_sock 结构体
	struct inet_connection_sock *icsk =
		inet_csk(sk); // 获取 inet_connection_sock 结构体
	const struct tcphdr *th = tcp_hdr(skb); // 获取 TCP 头部信息
	struct request_sock *req; // 存储请求的连接
	int queued = 0; // 是否排队标志
	bool acceptable; // 是否可以接受该数据包
	SKB_DR(reason); // 记录丢包原因

	switch (sk->sk_state) { // 根据 socket 的状态判断处理流程

	// TCP 连接关闭状态
	case TCP_CLOSE:
		SKB_DR_SET(reason, TCP_CLOSE); // 设置丢包原因
		goto discard; // 丢弃数据包

	// TCP 监听状态
	case TCP_LISTEN:
		if (th->ack)
			return 1; // 如果是 ACK 包，直接返回
		...
		if (th->syn) { // 如果是 SYN 包
			if (th->fin) { // 如果有 FIN 标志
				SKB_DR_SET(reason, TCP_FLAGS);
				goto discard; // 丢弃数据包
			}
			rcu_read_lock(); // 加锁防止并发访问
			local_bh_disable(); // 禁用底半部处理
			acceptable = icsk->icsk_af_ops->conn_request(sk, skb) >=
				     0; // 判断连接请求是否可接受
			local_bh_enable(); // 启用底半部处理
			rcu_read_unlock(); // 解锁

			if (!acceptable)
				return 1; // 如果连接请求不可接受，返回

			consume_skb(skb); // 消费掉数据包
			return 0; // 继续处理
		}
		SKB_DR_SET(reason, TCP_FLAGS);
		goto discard; // 丢弃数据包

	// TCP 客户端处于 SYN_SENT 状态
	case TCP_SYN_SENT:
		tp->rx_opt.saw_tstamp = 0; // 重置时间戳
		tcp_mstamp_refresh(tp); // 刷新时间戳
		queued = tcp_rcv_synsent_state_process(sk, skb,
						       th); // 处理 SYN 包
		if (queued >= 0)
			return queued;

		tcp_urg(sk, skb, th); // 处理 URG 包
		__kfree_skb(skb); // 释放 skb
		tcp_data_snd_check(sk); // 检查是否需要发送数据
		return 0;
	}
	...
	req = rcu_dereference_protected(
		tp->fastopen_rsk,
		lockdep_sock_is_held(sk)); // 获取快速打开的请求
	if (req) {
		bool req_stolen;

		WARN_ON_ONCE(sk->sk_state != TCP_SYN_RECV &&
			     sk->sk_state != TCP_FIN_WAIT1); // 状态检查
	...
	// step 5: 校验 ACK 字段
	acceptable = tcp_ack(sk, skb,
			     FLAG_SLOWPATH | FLAG_UPDATE_TS_RECENT |
				     FLAG_NO_CHALLENGE_ACK) > 0;

	if (!acceptable) {
		if (sk->sk_state == TCP_SYN_RECV)
			return 1; // 发送一个 RST 包
		tcp_send_challenge_ack(sk); // 发送挑战 ACK 包
		SKB_DR_SET(reason, TCP_OLD_ACK);
		goto discard; // 丢弃数据包
	}

	switch (sk->sk_state) {
		// TCP 状态从 SYN_RECV 变为 ESTABLISHED
	case TCP_SYN_RECV:
		tp->delivered++; // 更新已交付的数据
		if (!tp->srtt_us)
			tcp_synack_rtt_meas(sk, req); // 测量 SYN-ACK 的 RTT

		if (req) {
			tcp_rcv_synrecv_state_fastopen(sk); // 处理快速打开连接
		} else {
			tcp_try_undo_spurious_syn(sk); // 尝试恢复伪 SYN
			tp->retrans_stamp = 0;
			tcp_init_transfer(sk,
					  BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB,
					  skb); // 初始化数据传输
			WRITE_ONCE(tp->copied_seq,
				   tp->rcv_nxt); // 更新复制序列号
		}

		smp_mb(); // 内存屏障，确保顺序
		tcp_set_state(sk, TCP_ESTABLISHED); // 设置为 ESTABLISHED 状态
		sk->sk_state_change(sk); // 通知状态变化
		...
		tp->snd_una =
			TCP_SKB_CB(skb)->ack_seq; // 更新发送的未确认序列号
		tp->snd_wnd = ntohs(th->window)
			      << tp->rx_opt.snd_wscale; // 更新发送窗口
		tcp_init_wl(tp, TCP_SKB_CB(skb)->seq); // 初始化发送窗口

		if (tp->rx_opt.tstamp_ok)
			tp->advmss -= TCPOLEN_TSTAMP_ALIGNED; // 减去时间戳字段

		if (!inet_csk(sk)->icsk_ca_ops->cong_control)
			tcp_update_pacing_rate(sk); // 更新拥塞控制

		tp->lsndtime = tcp_jiffies32; // 更新发送时间

		tcp_initialize_rcv_mss(sk); // 初始化接收 MSS
		tcp_fast_path_on(tp); // 启用快速路径

		if (sk->sk_shutdown & SEND_SHUTDOWN)
			tcp_shutdown(sk, SEND_SHUTDOWN); // 关闭连接
	...
	case TCP_FIN_WAIT1: {
		...
		if (req)
			tcp_rcv_synrecv_state_fastopen(sk); // 快速打开处理

		if (tp->snd_una != tp->write_seq)
			break;

		tcp_set_state(sk, TCP_FIN_WAIT2); // 设置为 FIN_WAIT2 状态
		WRITE_ONCE(sk->sk_shutdown, sk->sk_shutdown | SEND_SHUTDOWN);

		sk_dst_confirm(sk);

		if (!sock_flag(sk, SOCK_DEAD)) {
			sk->sk_state_change(sk); // 唤醒挂起的关闭操作
			break;
		}
		...
		tmo = tcp_fin_time(sk); // 获取 FIN 的超时时间
	...
	if (sk->sk_state != TCP_CLOSE) {
		tcp_data_snd_check(sk); // 检查是否需要发送数据
		tcp_ack_snd_check(sk); // 检查是否需要发送 ACK
	}

	if (!queued) {
discard:
		tcp_drop_reason(sk, skb, reason); // 丢弃数据包
	}
	return 0;
...
// 	}
// 	switch (sk->sk_state) {
// 		//服务端收到客户端的SYN包并正在等待客户端确认，进入ESTABLISHED 状态
// 	case TCP_SYN_RECV:
// 	//更新发送窗口
...
{
	struct request_sock *req =
		reqsk_alloc(ops, sk_listener, attach_listener);
				 ...
				 const struct sk_buff *skb)
{
	if (tcp_sk(sk)->save_syn) {
		...
		saved_syn =
			kmalloc(struct_size(saved_syn, data, len), GFP_ATOMIC);
		if (saved_syn) {
		     ...
		     struct sk_buff *skb)
{
	...
	__u32 isn =
		TCP_SKB_CB(skb)
			->tcp_tw_isn; // 从TCP数据包中提取时间戳序列号（ISN）
	struct tcp_options_received tmp_opt; // 临时TCP选项结构体
	struct tcp_sock *tp = tcp_sk(sk); // 获取TCP套接字信息
	struct net *net = sock_net(sk); // 获取网络命名空间
	struct sock *fastopen_sk = NULL; // 用于存储快速打开的套接字
	struct request_sock *req; // 请求连接的套接字
	bool want_cookie = false; // 标记是否需要同步cookie
	struct dst_entry *dst; // 路由目标
	struct flowi fl; // 路由流信息
	u8 syncookies; // 是否启用同步cookie
...
//判断半连接队列是否已满
	if ((syncookies == 2 || inet_csk_reqsk_queue_is_full(sk)) && !isn) {
		//判断是否开启tcp_syncookies内核参数
		want_cookie = tcp_syn_flood_action(
			sk, rsk_ops->slab_name); 
		if (!want_cookie)
			goto drop; // 如果不需要cookie，则丢弃连接请求
	}

	//如果全连接队列满，有young_ack，那么直接丢弃
	if (sk_acceptq_is_full(sk)) {
		NET_INC_STATS(sock_net(sk),
	...
	if (!req)
		goto drop; // 如果请求套接字分配失败，丢弃连接请求

	// 设置同步cookie标志
	req->syncookie = want_cookie;
	// 设置该请求套接字的地址族相关操作
	tcp_rsk(req)->af_specific = af_ops;
	tcp_rsk(req)->ts_off = 0; // 初始化时间戳偏移

#if IS_ENABLED(CONFIG_MPTCP)
	// 如果启用了MPTCP，设置为非MPTCP请求
	tcp_rsk(req)->is_mptcp = 0;
#endif

	// 清空TCP选项并设置相关的TCP选项
	tcp_clear_options(&tmp_opt);
	tmp_opt.mss_clamp = af_ops->mss_clamp; // 设置最大报文段大小
	tmp_opt.user_mss = tp->rx_opt.user_mss; // 设置用户最大报文段大小
	tcp_parse_options(sock_net(sk), skb, &tmp_opt, 0,
			  want_cookie ? NULL : &foc);

	// 如果需要cookie且没有看到时间戳选项，则清空TCP选项
	if (want_cookie && !tmp_opt.saw_tstamp)
		tcp_clear_options(&tmp_opt);

	// 如果启用了SMC（共享内存通信），则禁用相关选项
	...
	inet_rsk(req)->no_srccheck =
		inet_sk(sk)->transparent; // 不进行源地址检查（透明套接字）
	...
	// 如果不需要cookie且ISN为空，则进行同步cookie和连接请求的验证
	if (!want_cookie && !isn) {
		int max_syn_backlog =
			READ_ONCE(net->ipv4.sysctl_max_syn_backlog);

		// 如果没有启用同步cookie，并且请求队列的剩余空间不足，则丢弃请求
		if (!syncookies &&
	...
	// 如果不需要cookie，记录连接请求并尝试快速打开
	if (!want_cookie) {
		tcp_reqsk_record_syn(sk, req, skb);
		fastopen_sk = tcp_try_fastopen(sk, skb, req, &foc, dst);
	}

	// 如果启用了快速打开，发送SYN-ACK并添加子套接字到接收队列
	if (fastopen_sk) {
		af_ops->send_synack(fastopen_sk, dst, &fl, req, &foc,
				    TCP_SYNACK_FASTOPEN, skb);
		...
		sk->sk_data_ready(sk); // 激活数据处理
		bh_unlock_sock(fastopen_sk);
		sock_put(fastopen_sk);
	} else {
		// 否则，设置请求的TFO（TCP Fast Open）监听器为false
		tcp_rsk(req)->tfo_listener = false;
		if (!want_cookie) {
			// 如果不需要cookie，初始化超时并添加请求到连接队列
			req->timeout = tcp_timeout_init((struct sock *)req);
			if (unlikely(!inet_csk_reqsk_queue_hash_add(
				    sk, req, req->timeout))) {
				reqsk_free(req);
				dst_release(dst);
				return 0;
...
drop_and_release:
	dst_release(dst); // 释放路由目标
drop_and_free:
	__reqsk_free(req); // 释放请求套接字
drop:
	tcp_listendrop(sk); // 丢弃连接请求
	return 0;
}
