...
{
	struct inet_hashinfo *hinfo = death_row->hashinfo;  // 获取哈希信息结构体
    struct inet_bind_hashbucket *head, *head2;           // 定义链表头指针
    struct inet_timewait_sock *tw = NULL;                // 记录时间等待队列中的套接字
    int port = inet_sk(sk)->inet_num;                    // 获取套接字的端口号
    struct net *net = sock_net(sk);                       // 获取网络命名空间
    struct inet_bind2_bucket *tb2;                        // 定义第二个绑定桶
    struct inet_bind_bucket *tb;                          // 定义绑定桶
    bool tb_created = false;                              // 标记绑定桶是否已创建
    u32 remaining, offset;                                // 端口剩余数量和偏移量
    int ret, i, low, high;                                // 函数返回值，遍历索引，低高端口
    int l3mdev;                                           // L3网络设备
    u32 index;                                            // 随机索引，用于扰动端口表
	//端口号存在
	if (port) {
		local_bh_disable();
		//检查连接是否已建立
	...
	if (likely(remaining > 1))
		remaining &= ~1U;
	//从table_perturb 全局表种获取一个随机值
	get_random_sleepable_once(table_perturb,
				  INET_TABLE_PERTURB_SIZE * sizeof(*table_perturb));
	//计算随机索引
	index = port_offset & (INET_TABLE_PERTURB_SIZE - 1);
	//offset是选择具体端口的偏移量，从table_perturb中获取一个值，加上偏移量的高32位
