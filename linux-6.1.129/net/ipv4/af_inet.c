...
int inet_listen(struct socket *sock, int backlog)
{
	...
	err = -EINVAL;
	//SS_UNCONNECTED套接字未连接SOCK_STREAM流式套接字（针对面向连接的TCP服务应用）
	if (sock->state != SS_UNCONNECTED || sock->type != SOCK_STREAM)
		goto out;

	old_state = sk->sk_state;
	//仅仅允许TCPF_CLOSE和TCP_LISTEN进入监听状态
	if (!((1 << old_state) & (TCPF_CLOSE | TCPF_LISTEN)))
		goto out;
    //WRITE_ONCE防止编译器优化/避免并发问题
			  ...
			  int addr_len, int flags, int is_sendmsg)
{
	...
	if (uaddr) {
		...
		if (uaddr->sa_family == AF_UNSPEC) {
			...
			//SS_UNCONNECTED:套接字未连接，SS_CONNECTED:套接字已连接，SS_CONNECTING:套接字正在连接，SS_DISCONNECTING:套接字正在断开连接
	...
	//获取套接字的发送超时时间，O_NONBLOCK非阻塞模式
	timeo = sock_sndtimeo(sk, flags & O_NONBLOCK);
	//套接字的状态为TCPF_SYN_SENT或TCPF_SYN_RECV
