				 ...
				 int rpf, struct in_device *idev, u32 *itag)
{
	 ...
	 */
	dev_match = dev_match ||
		    (res.type == RTN_LOCAL && dev == net->loopback_dev);
	if (dev_match) {
				 ...
				 struct fib_config *cfg)
{
	...
	if (rt->rt_gateway.sa_family == AF_INET && addr) {
		...
		if (rt->rt_flags & RTF_GATEWAY && addr_type == RTN_UNICAST)
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	nlmsg_for_each_attr(attr, nlh, sizeof(struct rtmsg), remaining) {
		...
		case RTA_METRICS:
			...
			break;
		case RTA_MULTIPATH:
			err = lwtunnel_valid_encap_type_attr(
				nla_data(attr), nla_len(attr), extack);
			if (err < 0)
				goto errout;
	...
	if (cfg->fc_nh_id) {
		if (cfg->fc_oif || cfg->fc_gw_family || cfg->fc_encap ||
		    cfg->fc_mp) {
			NL_SET_ERR_MSG(
				extack,
	...
	if (!ipv4_is_zeronet(prefix) && !(ifa->ifa_flags & IFA_F_SECONDARY) &&
		...
		if (!(ifa->ifa_flags & IFA_F_NOPREFIXROUTE))
			fib_magic(RTM_NEWROUTE,
				  dev->flags & IFF_LOOPBACK ? RTN_LOCAL :
							      RTN_UNICAST,
...
void fib_modify_prefix_metric(struct in_ifaddr *ifa, u32 new_metric)
{
	...
	/* add the new */
	fib_magic(RTM_NEWROUTE,
		  dev->flags & IFF_LOOPBACK ? RTN_LOCAL : RTN_UNICAST, prefix,
		  ifa->ifa_prefixlen, ifa, new_metric);

	/* delete the old */
	fib_magic(RTM_DELROUTE,
		  dev->flags & IFF_LOOPBACK ? RTN_LOCAL : RTN_UNICAST, prefix,
		  ifa->ifa_prefixlen, ifa, ifa->ifa_rt_priority);
}
		...
		if (ifa1->ifa_flags & IFA_F_SECONDARY) {
			...
			if (ifa1->ifa_mask == prim->ifa_mask &&
				...
				if (!prim1 ||
					...
					prim1 = inet_ifa_byprefix(
						in_dev, ifa1->ifa_address,
						ifa1->ifa_mask);
				if (!prim1)
					continue;
...
static void nl_fib_input(struct sk_buff *skb)
{
	...
	if (skb->len < nlmsg_total_size(sizeof(*frn)) ||
	    skb->len < nlh->nlmsg_len || nlmsg_len(nlh) < sizeof(*frn))
		return;
...
//ipv4转发表，为每个netns创建独立的ipv4路由表
static int __net_init ip_fib_net_init(struct net *net)
{
	int err;
	//计算路由表哈希桶数组大小，hlist_head 是 哈希链表头，用于存储 IPv4 FIB 表，FIB_TABLE_HASHSZ 256字节
	size_t size = sizeof(struct hlist_head) * FIB_TABLE_HASHSZ;
	//向 netns 注册 FIB 事件监听器
	err = fib4_notifier_init(net);
	if (err)
		return err;
//多路径路由：允许一条路由使用多个下一跳ECMP机制
#ifdef CONFIG_IP_ROUTE_MULTIPATH
	/* Default to 3-tuple */
	//计算多路径哈希值
...
#endif
	//计算 size 是否小于 L1 Cache 线大小，如果是，则扩大到 L1_CACHE_BYTES，避免多个CPU线程访问不同哈希桶时发生伪共享，导致CPU缓存一致性开销增加
...
static int __net_init fib_net_init(struct net *net)
{
...
#ifdef CONFIG_IP_ROUTE_CLASSID //计数器
	...
	if (error < 0)
		goto out;
	//初始化 Netlink FIB 查找机制
	error = nl_fib_lookup_init(net);
	if (error < 0)
		goto out_nlfl;
	//在 /proc 下注册 fib 相关的 proc 文件
...
//netns 创建时初始化 FIB 路由表
static struct pernet_operations fib_net_ops = {
	.init = fib_net_init, //初始化路由表
	.exit = fib_net_exit, //释放单个路由表
	.exit_batch = fib_net_exit_batch, //释放多个路由表
};
//初始化
void __init ip_fib_init(void)
{
	fib_trie_init();
	//在每个 netns 里注册 FIB 相关操作
