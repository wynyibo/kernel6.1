...
{
	//当前进程描述符task_struct拿到nsproxy命名空间，找到网络栈net_ns，获取进程网络命名空间
...
int __sys_listen(int fd, int backlog)
{
	...
	sock = sockfd_lookup_light(fd, &err, &fput_needed);
	if (sock) {
		//sysctl_somaxconn内核参数，用来设置每个socket监听队列的最大长度
		somaxconn = READ_ONCE(sock_net(sock->sk)->core.sysctl_somaxconn);
		//backlog应用层传入的参数，用来描述服务端TCP ESTABELLISHED状态对应的全连接队列长度
		if ((unsigned int)backlog > somaxconn)
			backlog = somaxconn;
	    //调用 SELinux / AppArmor 进行权限检查，防止未经授权的进程监听端口
		err = security_socket_listen(sock, backlog);
		if (!err)
		//数据包送到协议栈const struct proto_ops inet_stream_ops 找到调用inet_listen
		       ...
		       int addrlen, int file_flags)
{
	struct socket *sock;
	int err;
	//file转换成socket结构，一个fd对应一个socket结构体
	sock = sock_from_file(file);
	if (!sock) {
		err = -ENOTSOCK;
		goto out;
