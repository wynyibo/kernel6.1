...
struct fib_rt_info {
	...
	dscp_t dscp;
	u8 type;
	u8 offload : 1, trap : 1, offload_failed : 1, unused : 5;
};
...
//从 net_namespace 里的 fib_table_hash 获取指定 ID（id）的 fib_table
static inline struct fib_table *fib_get_table(struct net *net, u32 id)
{
	struct hlist_node *tb_hlist;
	struct hlist_head *ptr;
/*
	net->ipv4.fib_table_hash 是一个 哈希表，用于存储不同 fib_table
	本地路由表（RT_TABLE_LOCAL）的 ID 为 255
	主路由表（RT_TABLE_MAIN）的 ID 为 254

*/
	ptr = id == RT_TABLE_LOCAL ?
		      &net->ipv4.fib_table_hash[TABLE_LOCAL_INDEX] :
		      &net->ipv4.fib_table_hash[TABLE_MAIN_INDEX];
	//获取 ptr 链表的第一个节点（hlist_node）
...
//在 net 命名空间的 RT_TABLE_MAIN 路由表中查找 flp 对应的路由。
			     ...
			     struct fib_result *res, unsigned int flags)
{
	...
	rcu_read_lock();
	//获取net 命名空间的 RT_TABLE_MAIN 路由表
	tb = fib_get_table(net, RT_TABLE_MAIN);
	if (tb)
		//路由匹配，tb：要查找的 fib_table。flp：匹配条件（目的 IP、源 IP、出接口等）。res：存储匹配的下一跳 fib_result。flags | FIB_LOOKUP_NOREF：FIB_LOOKUP_NOREF：不增加 fib_info 的引用计数。
						 ...
						 struct flow_keys *flkeys)
{
	...
	__skb_flow_dissect(net, skb, &flow_keys_dissector, flkeys, NULL, 0, 0,
			   0, flag);
...
#define FIB_MULTIPATH_HASH_FIELD_OUTER_MASK                                  \
	(FIB_MULTIPATH_HASH_FIELD_SRC_IP | FIB_MULTIPATH_HASH_FIELD_DST_IP | \
...
int fib_nh_init(struct net *net, struct fib_nh *fib_nh, struct fib_config *cfg,
		int nh_weight, struct netlink_ext_ack *extack);
void fib_nh_release(struct net *net, struct fib_nh *fib_nh);
int fib_nh_common_init(struct net *net, struct fib_nh_common *nhc,
		       struct nlattr *fc_encap, u16 fc_encap_type, void *cfg,
