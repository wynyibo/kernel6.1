...
//veth设备的私有数据结构private data,每一个veth设备都有一个veth_priv结构
struct veth_priv {
	struct net_device __rcu	*peer;//对端设备peer（dev和peer通过该指针完成结对）
	atomic64_t		dropped;//veth设备丢弃的数据包数量
	struct bpf_prog		*_xdp_prog;//xdp程序
	struct veth_rq		*rq;//veth的请求队列（多队列）
	unsigned int		requested_headroom;//veth设备请求的头部空间大小
};
...
{
	//数据包转发到对端设备__dev_forward_skb，对端开启XDP则调用veth_xdp_rx，普通数据包调用__netif_rx
			...
			struct netlink_ext_ack *extack)
{
	 ...
	 */
	//data[VETH_INFO_PEER]存在，说明用户指定了veth设备对端属性
	if (data != NULL && data[VETH_INFO_PEER] != NULL) {
		struct nlattr *nla_peer;

		nla_peer = data[VETH_INFO_PEER];// 获取 veth 对端的属性信息
		ifmp = nla_data(nla_peer); // 提取对端的 ifinfomsg 结构
		//解析对端的属性信息
		...
		tbp = peer_tb;// 使用解析后的 peer_tb 作为参数
	} else {
		ifmp = NULL;
		tbp = tb; // 直接使用传入的 tb
	}
	//用户在ip link add 时指定了对端设备
	...
	//ifmp不存在，说明用户没有指定对端设备的属性信息且peer设备没有指定MAC地址
	if (!ifmp || !tbp[IFLA_ADDRESS])
	//随机生成MAC地址
		eth_hw_addr_random(peer);
	//ifmp存在
	if (ifmp && (dev->ifindex != 0))
	//peer的ifindex设置为dev的ifindex
		peer->ifindex = ifmp->ifi_index;
	//让peer设备继承主设备dev的最大TSO（大包分片卸载）参数
	...
	//关闭peer设备的链路状态carrier on（链路接通）：设备可以发送和接收数据包。carrier off（链路断开）：设备无法传输数据。
	netif_carrier_off(peer);
	//解析并应用 ifmp（ifinfomsg）中的 netlink 参数，比如 MTU、MAC 地址等。
